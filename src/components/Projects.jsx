...
const Projects = () => {
  ...
  useEffect(() => {
    // When the tab changes, reset the loader
    setLoading(true);
    setLoadedCount(0);
  }, [activeTab]);
  ...
  const handleImageLoad = () => {
    setLoadedCount((prev) => {
      ...
      // Once all visible images are loaded, hide the loader
      if (newCount >= Math.min(3, projects.length)) {
        setLoading(false);
      }
      return newCount;
    });
  ...
  const videoSwipeHandlers = useSwipeable({
    onSwipedLeft: () => setFeaturedIndex((prev) => (prev + 1) % featuredVideos.length),
    onSwipedRight: () => setFeaturedIndex((prev) => (prev - 1 + featuredVideos.length) % featuredVideos.length),
  ...
  return (
    <>
      ...
      {/* Featured Videos Section - can remain as you have it */}
    </>
  );
};
